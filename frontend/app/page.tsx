...
import { 
  ...
  UploadCloud, Link as LinkIcon, Check, X
} from "lucide-react";
...
export default function NexusApp() {
  ...
  // FIXED: Only fetch if user exists to prevent API errors on load
  const fetchProducts = () => {
    if (!user) return; 
  ...
  const handleLoginSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      const fakeUser = { name: decoded.name, email: decoded.email, picture: decoded.picture, role: "Manager" };
      setUser(fakeUser);
      ...
      <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
        <div className="flex h-screen items-center justify-center bg-gray-900 relative overflow-hidden font-sans">
          ...
          <div className="z-10 bg-white/10 backdrop-blur-lg p-10 rounded-2xl border border-white/20 shadow-2xl text-center max-w-md w-full">
            ...
            <p className="text-gray-300 mb-8">Unified Retail Intelligence</p>
            <div className="flex justify-center"><GoogleLogin onSuccess={handleLoginSuccess} onError={() => alert('Login Failed')} theme="filled_black" shape="pill" size="large" /></div>
            
            {/* --- DEV BYPASS BUTTON --- */}
            <div className="mt-4 flex justify-center">
              <button 
                onClick={() => {
                  const fakeUser = { name: "Mbuso (Dev)", email: "dev@nexus.com", picture: "", role: "Manager" }; // Empty picture to test fallback
                  setUser(fakeUser);
                  localStorage.setItem("nexus_user", JSON.stringify(fakeUser));
                  if (!localStorage.getItem("nexus_onboarding_complete")) setShowOnboarding(true);
                }} 
            ...
            <p className="text-xs text-gray-400 mt-6">Enterprise Access Only</p>
          </div>
        </div>
      </GoogleOAuthProvider>
    );
...
// --- ONBOARDING WIZARD ---
function OnboardingWizard({ onComplete, setStoreName }: any) {
  const [step, setStep] = useState(1);
  const [loading, setLoading] = useState(false);
  const handleStoreSetup = () => { setStep(2); };
    ...
    setTimeout(async () => {
      try {
        await fetch("http://127.0.0.1:8000/products/", {
          ...
          body: JSON.stringify({ sku: "IMP-001", name: "Imported Item (Excel)", cost_price: 50, selling_price: 100, stock_quantity: 500, category: "Bulk Import" })
        });
      } catch (e) { console.error("Simulated upload failed - likely backend offline"); }
      setLoading(false); setStep(3);
    }, 2000);
  };
  const handleIQSync = () => { setLoading(true); setTimeout(() => { setLoading(false); setStep(4); }, 2000); };

  return (
    <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center">
      <div className="bg-white w-[600px] rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-300">
        <div className="bg-indigo-600 p-8 text-white text-center">
          <div className="mx-auto w-12 h-12 bg-white/20 rounded-full flex items-center justify-center mb-4 font-bold text-xl">{step}/4</div>
          <h2 className="text-2xl font-bold">Welcome to NexusRetail</h2>
          <p className="text-indigo-100 mt-2">Let's get your store running in minutes.</p>
        </div>
        <div className="p-10 flex-1">
          {step === 1 && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-gray-800">1. Store Setup</h3>
              <div><label className="block text-sm font-bold text-gray-600 mb-2">Store Name</label><input placeholder="My Retail Store" className="w-full border p-3 rounded-xl" onChange={(e) => { setStoreName(e.target.value); localStorage.setItem("storeName", e.target.value); }} /></div>
              <div><label className="block text-sm font-bold text-gray-600 mb-2">Currency</label><select className="w-full border p-3 rounded-xl"><option>ZAR (South Africa)</option><option>USD (United States)</option></select></div>
              <button onClick={handleStoreSetup} className="w-full bg-indigo-600 text-white p-4 rounded-xl font-bold mt-4">Next Step</button>
            </div>
          )}
          {step === 2 && (
            <div className="space-y-6 text-center">
              <h3 className="text-xl font-bold text-gray-800">2. Import Stock</h3>
              <p className="text-gray-500">Upload your Excel spreadsheet to auto-populate inventory.</p>
              <div className="border-2 border-dashed border-gray-300 rounded-2xl p-10 flex flex-col items-center justify-center bg-gray-50 hover:bg-indigo-50 hover:border-indigo-300 transition-colors cursor-pointer" onClick={handleExcelUpload}>
                {loading ? <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-indigo-600"></div> : <><UploadCloud size={48} className="text-gray-400 mb-4"/><p className="font-bold text-gray-600">Click to Upload .XLSX</p></>}
              </div>
              {loading && <p className="text-xs text-indigo-600 font-bold">AI is categorizing products...</p>}
            </div>
          )}
          {step === 3 && (
            <div className="space-y-6 text-center">
              <h3 className="text-xl font-bold text-gray-800">3. Connect IQ Retail</h3>
              <p className="text-gray-500">Sync with your existing legacy system.</p>
              <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100 flex items-center gap-4">
                <div className="bg-white p-3 rounded-lg shadow-sm font-bold text-blue-800">IQ</div>
                <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">{loading ? <div className="h-full bg-blue-500 animate-pulse w-full"></div> : <div className="h-full w-0"></div>}</div>
                <div className="bg-white p-3 rounded-lg shadow-sm"><Sparkles className="text-indigo-600"/></div>
              </div>
              <button onClick={handleIQSync} disabled={loading} className="w-full bg-blue-600 text-white p-4 rounded-xl font-bold mt-4 flex items-center justify-center gap-2">{loading ? "Syncing..." : <><LinkIcon size={18}/> Connect API</>}</button>
            </div>
          )}
          {step === 4 && (
            <div className="space-y-6 text-center">
              <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto text-green-600 mb-6"><Check size={40} /></div>
              <h3 className="text-2xl font-bold text-gray-800">Setup Complete!</h3>
              <p className="text-gray-500">Your store is configured, stock is imported, and AI is active.</p>
              <button onClick={onComplete} className="w-full bg-gray-900 text-white p-4 rounded-xl font-bold mt-6 hover:scale-105 transition-transform">Enter Dashboard</button>
            </div>
          )}
        </div>
      </div>
...
// ==============================================
// DASHBOARD & SUB-VIEWS (Unchanged Logic)
// ==============================================
function DashboardView({ products, navigate }: { products: Product[], navigate: Function }) {
  ...
  return (
    <div className="p-10 max-w-7xl mx-auto">
      <header className="mb-10 flex justify-between items-end"><div><h2 className="text-4xl font-bold text-gray-900 tracking-tight">Operational Dashboard</h2><p className="text-gray-500 mt-2 text-lg">Overview & Quick Actions</p></div><div className="hidden md:block"><span className="bg-indigo-50 text-indigo-700 px-4 py-2 rounded-full text-sm font-bold">{date || "Loading..."}</span></div></header>
      <section className="mb-12 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div onClick={() => navigate('add-product')} className="bg-white p-6 rounded-[20px] shadow-sm border border-gray-100 cursor-pointer hover:border-indigo-500 transition-all hover:translate-y-[-2px] flex flex-col"><div className="w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 mb-4"><Plus size={24}/></div><h4 className="font-bold text-xl">Add Product</h4> <p className="text-gray-500 mt-2">Register new stock items.</p></div>
      </section>
      <h3 className="text-lg font-bold text-gray-700 mb-5">All Actions</h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4"><ActionBtn icon={<Package/>} label="Inventory Count" onClick={() => navigate('inventory')} /><ActionBtn icon={<Plus/>} label="Add New Product" onClick={() => navigate('add-product')} /><ActionBtn icon={<FileText/>} label="Sales Reports" onClick={() => navigate('reports')} /><ActionBtn icon={<Users/>} label="Staff Management" onClick={() => navigate('staff')} /><ActionBtn icon={<Truck/>} label="Suppliers" onClick={() => navigate('suppliers')} /><ActionBtn icon={<Settings/>} label="Settings" onClick={() => navigate('settings')} /></div>
    </div>
  );
}

function POSView({ products, refresh }: { products: Product[], refresh: Function }) {
  const [cart, setCart] = useState<CartItem[]>([]); const [predictions, setPredictions] = useState<Record<string, AIPrediction>>({}); const [search, setSearch] = useState("");
