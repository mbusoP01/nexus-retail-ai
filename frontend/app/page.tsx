...
function InventoryView({ products, refresh }: any) {
  const updateStock = async (sku: string, newQty: number) => {
    await fetch(`${API_BASE_URL}/products/${sku}/stock`, {
          ...
          <tbody className="divide-y divide-gray-100">
            {products.map((p: Product) => (
              <tr key={p.sku} className="hover:bg-gray-50">
                ...
                <td className="p-5">
                  {p.images && p.images.length > 0 ? (
                    <img src={p.images[0].image_url} alt={p.name} className="w-12 h-12 object-cover rounded-lg border border-gray-200" />
                  ) : (
                ...
                <td className="p-5 font-mono text-xs text-gray-500">{p.sku}</td>
                <td className="p-5 font-bold text-gray-900">{p.name}</td>
                <td className="p-5">
                  <input 
                    type="number" 
                    defaultValue={p.stock_quantity} 
                    onBlur={(e) => updateStock(p.sku, parseInt(e.target.value))} 
                    className="w-24 border border-gray-200 rounded-lg p-2 text-center font-bold text-indigo-600 focus:border-indigo-500 outline-none" 
                  />
                </td>
                <td className="p-5">
                  {p.stock_quantity < 10 ? 
                    <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-xs font-bold">Low Stock</span> : 
                    <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold">Good</span>
                  }
                </td>
              </tr>
            ))}
